...
/*********************************************************************
 * <p>The class of {@code UsePreOrderIdentHandler} is used to abstractly define
 ...
 */
public class UsePreOrderIdentHandler extends AHandler {
     ...
     * - the logger of the {@link UsePreOrderIdentHandler class</p>
     */
    ...
    public UsePreOrderIdentHandler(HandlerContainer container) {
    ...
    public boolean handle(IConnection connection, IMessage message) {
        ...
        if(message instanceof UsePreOrderIdent) {

            IService service = connection.getConnectionManager().getService();

            if(service.getServiceType().equals(EServiceType.CLIENT)) {

                Client.getInstance().getPreOrder().setIdentificator(
                        ((UsePreOrderIdent) message).getContent()
                );
                return true;
            }
        }
    ...
    public IHandler copy(HandlerContainer container) {
        ...
        return new UsePreOrderIdentHandler(container);
    }
